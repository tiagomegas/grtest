...
export default class Input extends Component {
  ...
  static defaultProps = {
    ...
    let optional;
    if ("maxValue" === this.props.format && null !== this.props.maxValue) {
      optional = this.props.maxValue;
    }
		if (this.props.required) {
			if (isEmpty(val)) {
				this.setState({valid:false});
        this.props.onFieldChange(e,false);
				return;
			}
      let valid = this.validateFormat(this.props.format, val, optional);
      this.setState({valid:valid});
  		this.props.onFieldChange(e,valid);
		} else {
      this.setState({valid:this.validateFormat(this.props.format, val, optional)});
	...
	validateFormat(format, val, optional) {

		switch (format) {
			case 'email' : return validations.isEmail('',val);
      case 'numeric' : return validations.isNumeric('',val);
      case 'maxValue' : return validations.maxValue(optional,val);
			default : return true;
		}
	}
    ...
    let {errorMessage = 'required', onFieldChange, classes, rounded,valid, ...rest} = this.props;
    let error = this.state.valid && this.props.valid ? null : <p className="alert alert-danger">{errorMessage}</p>
		return (
...
var validations = {
  ...
  maxValue: function (maxValue, value) {
    return validations.isNumeric('', value) && maxValue>=value;
  }
};
